...
listen_ip = config['tcp_server']['listen_ip']
listen_port = int(config['tcp_server']['listen_port'])
...
class ProducerWorker(threading.Thread):
    ...
    def run(self):
        ...
        # TARGET_BATCH_BYTES = 512 * 1024  # 512 KB
        ...
        #
        # def get_size(msg):
        #     return len(msg) if isinstance(msg, (bytes, bytearray)) else len(str(msg).encode())

        while self.running:
            try:
                msg_str = self.queue.get(timeout=0.01)
                self.queue.task_done()

                if message_format.lower() == 'protobuf':
                    message_to_proto = parse_fix_message_to_proto(msg_str)
                    # print("==",message_to_proto)
                    message = message_to_proto.SerializeToString()
                else:
                    message = convert_string_to_json(msg_str)
                    message = json.dumps(message)

                # partition_key = str(hash(msg_str) % 100)
                batch.append(message)
                # batch_bytes += get_size(message)

                # if len(batch) >= 500 or (time.time() - last_flush) >= 0.05:
    ...
    def send_batch(self, batch):
        ...
        for message in batch:
            try:
                ...
                logger.error(f"[Worker-{self.worker_id}]- {self.topic} Kafka error: {e}")
...
def multi_threaded_client(connection):
    ...
    try:
        ...
        try:
            ...
            pass
        logger.info(f"Connection closed for {client_address}. Active clients: {ThreadCount}")
...
def main():
    ...
    try:
        ...
        # send_mail("ALERT! from Kafka Producer", "Application has stopped forcefully")
